...
export default class Home extends Component {
    ...
    constructor(props) {
        ...
        Tts.stop(); Tts.speak("퀴즈모드 스터디모드 점자번역모드 중에 선택해주세요",{language:"ko"});
...
onSpeechPartialResults(e) {
    ...
    if (speech.includes("스터디")) {               
        this.props.navigation.navigate('Study', {deviceinfo: devices[0]}); 
        Tts.stop(); Tts.speak("스터디모드"); 
        this.setState({
            results: '',
        });
    } else if (speech.includes("퀴즈")) {
        this.props.navigation.navigate('Quiz', {deviceinfo: devices[0]}); 
        Tts.stop(); Tts.speak("퀴즈모드");        
        this.setState({
            results: '',
        });
    }
    else if (speech.includes("번역")) {
        this.props.navigation.navigate('Translate', {deviceinfo: devices[0]}); 
        Tts.stop(); Tts.speak("번역모드");
        this.setState({
            results: '',
        });
    }
            ...
            <Button
                title="Write packets"
                          style={{
                            ...
                            textStyle={{ color: "#fff" }}
                            onPress={() =>
                            this.writePackets(id,"This is the test message!Does it work? Tell me it works!F")
                          }
                        />
                      </React.Fragment> )}      
            <Button
