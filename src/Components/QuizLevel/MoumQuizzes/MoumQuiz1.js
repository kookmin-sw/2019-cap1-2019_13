...
export default class MoumQuiz1 extends Component {
    constructor(props) {
        super(props);
        this.mounted = false;
        this.state = {
        ...
        Tts.speak("모음퀴즈입니다 모음 어 는 몇번인가요",{language:"ko"});
    ...
    onSpeechPartialResults(e) {
        ...
        if (speech.includes("문제")) {
            Tts.speak("모음 어 는 몇번인가요",{language:"ko"});
            this.setState({
                results: '',
            ...
            <View style={{ flex: 1, backgroundColor : '#f5fcff' }}>
                ...
                <TouchableOpacity style={{ flex: 0.9 }}>
                    <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                        <TouchableOpacity style={styles.buttonst} onPress={() => {Tts.speak("모음 어 는 몇번인가요", {language:"ko"});}} >
                            <Text style ={{fontSize:30, color:'white'}}>문제듣기</Text>
                        </TouchableOpacity>

                        <Text style={{color: 'black', fontSize: 75}}>Quiz</Text>
                        
                        <TouchableOpacity style={styles.buttonst} onPress={() => {Tts.speak("다음문제", {language:"ko"}); this.props.navigation.navigate('MoumQuiz2');}} >
                            <Text style ={{fontSize:30, color:'white'}}>다음</Text>
                        </TouchableOpacity>
                    </View>
                    
                    <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                        <TouchableOpacity style={styles.buttonst2} onPress={() =>  {goBack(); Tts.speak("시작페이지로 가기", {language:"ko"});}}  >
                            <Text style ={{fontSize:20, color:'white' }}>시작페이지</Text>
                        </TouchableOpacity>
                                    
                        <TouchableOpacity style={styles.buttonst2} onPress={() => {Tts.stop(); Tts.speak("정답은 1번입니다.",{language:"ko"}); }}  >
                            <Text style ={{fontSize:20, color:'white' }}>정답듣기</Text>
                        </TouchableOpacity>
                    </View>

                    <View style={{justifyContent: 'center', alignItems: 'center'}}>
                        <Text style = {{fontSize:60, color: 'black'}}>모음 'ㅓ'는?</Text> 
                    </View>

                    <View style={{flexDirection: 'row', justifyContent: 'center', marginTop: 50}}>
                        <TouchableOpacity style={styles.button} onPress={() => {Tts.speak("1번",{language:"ko"}); this.write(device_dot_in_moumquiz.id, "1011100F");}}>
                            <Text style ={{fontSize:70,color:'white' }}>1</Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={styles.button} onPress={() => {Tts.stop(); Tts.speak("2번",{language:"ko"}); this.write(device_dot_in_moumquiz.id, "1111100F");}}>
                            <Text style ={{fontSize:70,color:'white'}}>2</Text>
                        </TouchableOpacity>
                            
                        <TouchableOpacity style={styles.button} onPress={() => {Tts.stop(); Tts.speak("3번",{language:"ko"}); this.write(device_dot_in_moumquiz.id, "1010101F");}}>
