...
export default class MoumQuiz2 extends Component {
    constructor(props) {
        ...
        Tts.speak("모음퀴즈입니다 모음 와 는 몇번인가요",{language:"ko"});
    ...
    onSpeechPartialResults(e) {
        ...
        if (speech.includes("문제")) {
            Tts.speak("모음 와 는 몇번인가요",{language:"ko"});
            this.setState({
                results: '',
            ...
            this.write(device_dot_in_moumquiz.id, "1010100F");
        }
        else if (speech.includes("2번") || speech.includes("이번")) {
            this.write(device_dot_in_moumquiz.id, "1111001F");
        }
            ...
            <View style={{ flex: 1, backgroundColor : '#f5fcff' }}>
                ...
                <TouchableOpacity style={{ flex: 0.9 }}>
                    <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                        <TouchableOpacity style={styles.buttonst} onPress={() => {Tts.speak("모음 와 는 몇번인가요", {language:"ko"});}} >
                    ...
                    <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                        <TouchableOpacity style={styles.buttonst2} onPress={() => {Voice.destroy().then(Voice.removeAllListeners); this.mounted = false; this.props.navigation.pop(2);}}  >
                    ...
                    <View style={{flexDirection: 'row', justifyContent: 'center', marginTop: 50}}>
                        <TouchableOpacity style={styles.button} onPress={() => {Tts.speak("1번",{language:"ko"}); this.write(device_dot_in_moumquiz.id, "1010100F");}}>
                            <Text style ={{fontSize:70,color:'white' }}>1</Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={styles.button} onPress={() => {Tts.speak("2번",{language:"ko"}); this.write(device_dot_in_moumquiz.id, "1111001F");}}>
                            <Text style ={{fontSize:70,color:'white'}}>2</Text>
                        </TouchableOpacity>
                            
                        <TouchableOpacity style={styles.button} onPress={() => {Tts.speak("3번",{language:"ko"}); this.write(device_dot_in_moumquiz.id, "1111100F");}}>
