...
export default class a_quiz_two extends Component {
    constructor(props) {
        super(props);
        this.mounted = false;
        this.state = {
        ...
        Tts.speak("자음퀴즈입니다 초성디귿은 몇번인가요?",{language:"ko"});
    ...
    onSpeechPartialResults(e) {
        ...
        if (speech.includes("문제")) {
            Tts.speak("초성 디귿은 무엇인가요?",{language:"ko"});
            this.setState({
                results: '',
            ...
            this.props.navigation.navigate('JaumQuiz3');
            this.setState({
                results: '',
            ...
            <View style={{ flex: 1, backgroundColor : '#f5fcff' }}>
                ...
                <TouchableOpacity style={{ flex: 0.9 }}>
                    <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                        <TouchableOpacity style={styles.buttonst} onPress={() => {Tts.speak("초성 디귿은 무엇인가요?", {language:"ko"});}} >
                            <Text style ={{fontSize:30, color:'white'}}>문제듣기</Text>
                        </TouchableOpacity>

                        <Text style={{color: 'black', fontSize: 75}}>Quiz</Text>
                        
                        <TouchableOpacity style={styles.buttonst} onPress={() => {Tts.speak("다음문제", {language:"ko"}); this.props.navigation.navigate('JaumQuiz3');}} >
                            <Text style ={{fontSize:30, color:'white'}}>다음</Text>
                        </TouchableOpacity>
                    </View>
                    
                    <View style={{flexDirection: 'row', justifyContent: 'center'}}>
                        <TouchableOpacity style={styles.buttonst2} onPress={() =>  {Tts.speak("시작페이지로가기", {language:"ko"}); Voice.destroy().then(Voice.removeAllListeners); this.mounted = false; this.props.navigation.pop(2);}}  >
                    ...
                    <View style={{justifyContent: 'center', alignItems: 'center'}}>
                        <Text style = {{fontSize:60, color: 'black'}}>초성 "ㄷ"는?</Text> 
                    </View>

                    <View style={{flexDirection: 'row', justifyContent: 'center', marginTop: 50}}>
                        <TouchableOpacity style={styles.button} onPress={() => {Tts.speak("일번",{language:"ko"}); this.write(device_dot_in_jaumquiz.id, "1011100F");}}>
                            <Text style ={{fontSize:70,color:'white' }}>1</Text>
                        </TouchableOpacity>

                        <TouchableOpacity style={styles.button} onPress={() => {Tts.speak("이번",{language:"ko"}); this.write(device_dot_in_jaumquiz.id, "1001100F");}}>
                            <Text style ={{fontSize:70,color:'white'}}>2</Text>
                        </TouchableOpacity>
                            
                        <TouchableOpacity style={styles.button} onPress={() => {Tts.speak("삼번",{language:"ko"}); this.write(device_dot_in_jaumquiz.id, "1010100F");}}>
