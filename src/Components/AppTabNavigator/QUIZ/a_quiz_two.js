...
export default class a_quiz_two extends Component {
    ...
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center',backgroundColor :'bisque'  }}>
           ...
           {this.state.partialResults.map((result, index) => {
               ...
               <Text key={`partial-result-${index}`} >{result == "문제"?  Tts.speak("자음퀴즈입니다. 초성 'ㄷ'은 무엇인가요?",{language:"ko"}):''}{result == "다음"? this.props.navigation.navigate('a_quiz_three') :''}

               {result == "정답"? Tts.speak("정답은 3번입니다.",{language:"ko"}):''}{result == "시작"? this.props.navigation.navigate('quizstart'):''}
      ...
      <TouchableOpacity style={{backgroundColor:'black', position:'absolute',right:-20,paddingTop:10,paddingBottom:10, paddingLeft:20, paddingRight:20, borderRadius: 10 ,  bottom :80   }}   
       ...
       AppRegistry.registerComponent('a_quiz_two', () => a_quiz_two);