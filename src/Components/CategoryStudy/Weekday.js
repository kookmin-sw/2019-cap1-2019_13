...
class Weekday extends Component {
    ...
    onSpeechPartialResults(e) {
        console.log('here weekday');
        var speech = e.value[0].split(" ").slice(-1)[0];
        console.log(speech);
        const {goBack} = this.props.navigation;
        const device_dot_in_weekday = this.props.navigation.getParam('deviceinfo2', 'cantread');

        if (speech.includes("뒤로")) {
            goBack();
        }
        else if (speech.includes("월")) {
            this.write(device_dot_in_weekday.id, "2111100010000F");
            this.setState({
                results: '',
            });
        } 
        else if (speech.includes("화")) {
            this.write(device_dot_in_weekday.id, "2010110111001F");
            this.setState({
                results: '',
            });
        }
        else if (speech.includes("수")) {
            this.write(device_dot_in_weekday.id, "2000001101100F");
            this.setState({
                results: '',
            });
        }
        else if (speech.includes("금")) {
            this.write(device_dot_in_weekday.id, "3000100010101010001F");
            this.setState({
                results: '',
            });
        }
    ...
    render() {
        const {goBack} = this.props.navigation;
        const device_dot_in_weekday = this.props.navigation.getParam('deviceinfo2', 'cantread');
        console.log("device info in weekday: ", device_dot_in_weekday.id);
            ...
            <View style={styles.container}>
                ...
                showsVerticalScrollIndicator={false}>
                    <View style={styles.card}>
                        ...
                        <TouchableOpacity onPress={()=>{Tts.speak("월", { language: 'ko', rate: 0.75 }); this.write(device_dot_in_weekday.id, "2111100010000F");}} style={styles.button}>
                    ...
                    <View style={styles.card}>
                        ...
                        <TouchableOpacity onPress={()=>{Tts.speak("화", { language: 'ko', rate: 0.75 }); this.write(device_dot_in_weekday.id, "2010110111001F");}} style={styles.button}>
                    ...
                    <View style={styles.card}>
                        ...
                        <TouchableOpacity onPress={()=>{Tts.speak("수", { language: 'ko', rate: 0.75 }); this.write(device_dot_in_weekday.id, "2000001101100F");}} style={styles.button}>
                    ...
                    <View style={styles.card}>
                        ...
                        <TouchableOpacity onPress={()=>{Tts.speak("금", { language: 'ko', rate: 0.75 }); this.write(device_dot_in_weekday.id, "3000100010101010001F");}} style={styles.button}>
